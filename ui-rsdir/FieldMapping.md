...
| `disabled` | boolean | No | Renders a greyed-out field label with no value or editing controls. Supported on top-level mappings, `subMap` children, and `objectMap` children. Disabled fields are excluded from validation but retain their normal serialized value. |
| `onlyOne` | boolean | No | For a `subField`, allows at most one direct `subMap` child to be selected. The editor renders a child selector and omits unselected children from the saved value. Combine with `required` to require exactly one selection. |
